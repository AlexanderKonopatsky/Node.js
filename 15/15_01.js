...
http.createServer(function (request, response) {
    if (request.method == 'GET') {
        if (url.parse(request.url).pathname === '/api/faculties') {
                MongoClient.connect("mongodb+srv://Alex:Alex@cluster0.dzody.mongodb.net/BSTU?retryWrites=true&w=majority", { useNewUrlParser: true, useUnifiedTopology: true }, function (err, client) {
        ...
        if (url.parse(request.url).pathname === '/api/faculties') {
            request.on('data', (data) => {
                MongoClient.connect("mongodb+srv://Alex:Alex@cluster0.dzody.mongodb.net/BSTU?retryWrites=true&w=majority", { useNewUrlParser: true, useUnifiedTopology: true }, function (err, client) {
                    var db = client.db('BSTU');
                    db.collection('faculty', function (err, collection) {
                        collection.insertOne({ faculty: JSON.parse(data.toString()).faculty, faculty_name: JSON.parse(data.toString()).faculty_name }, function (err, doc) {
                        });
                        if (err) {
            ...
            request.on('data', (data) => {
                MongoClient.connect("mongodb+srv://Alex:Alex@cluster0.dzody.mongodb.net/BSTU?retryWrites=true&w=majority", { useNewUrlParser: true, useUnifiedTopology: true }, function (err, client) {
                    var db = client.db('BSTU');
                    db.collection('pulpit', function (err, collection) {
                        collection.insertOne({ pulpit: JSON.parse(data.toString()).pulpit, pulpit_name: JSON.parse(data.toString()).pulpit_name, faculty: JSON.parse(data.toString()).faculty }, function (err, doc) {
                        });
                        if (err) {
                            console.log('Error inserting item');
                            response.send('Error inserting item');
        ...
        if (url.parse(request.url).pathname === '/api/faculties') {
            request.on('data', (data) => {
                MongoClient.connect("mongodb+srv://Alex:Alex@cluster0.dzody.mongodb.net/BSTU?retryWrites=true&w=majority", { useNewUrlParser: true, useUnifiedTopology: true }, function (err, client) {
                    var db = client.db('BSTU');
                    db.collection('faculty', function (err, collection) {
                        collection.updateOne({ faculty: JSON.parse(data.toString()).faculty }, { $set: { faculty_name: JSON.parse(data.toString()).faculty_name } },
                            function (err, result) {
                                if (err) {
            ...
            request.on('data', (data) => {
                MongoClient.connect("mongodb+srv://Alex:Alex@cluster0.dzody.mongodb.net/BSTU?retryWrites=true&w=majority", { useNewUrlParser: true, useUnifiedTopology: true }, function (err, client) {
                    var db = client.db('BSTU');
                    db.collection('pulpit', function (err, collection) {
                        collection.updateOne({ pulpit: JSON.parse(data.toString()).pulpit }, { $set: { pulpit_name: JSON.parse(data.toString()).pulpit_name, faculty: JSON.parse(data.toString()).faculty } },
                            function (err, result) {
                                if (err) {
                                    console.log('Error updating item');
                                    response.end('Error updating item');
